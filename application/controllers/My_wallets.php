...
class My_wallets extends Secure_area implements iData_controller {
    ...
    function _dt_wallets()
    {
        ...
        foreach ($wallets->result() as $wallet)
        {
            ...
            $actions = "<a href='javascript:void(0)' data-href='". site_url("my_wallets/view/" . $wallet->wallet_id) ."' class='btn-xs btn-edit-wallet btn-primary btn' data-wallet-id='" . $wallet->wallet_id . "' title='Update'><span class='fa fa-pencil'></span></a> ";
            ...
            $data_row["name"] = $wallet->name;
            $data_row["amount"] = to_currency($wallet->amount, 1);
            $data_row["description"] = str_replace("In payments for", ktranslate2("In payments for"), $wallet->descriptions);
            $data_row["wallet_type"] = ktranslate2(ucwords($wallet_type));
    ...
    {
        $trans_date = $this->config->item('date_format') == 'd/m/Y' ? strtotime(uk_to_isodate($this->input->post("trans_date"))) : strtotime($this->input->post("trans_date"));
        ...
        $wallet_data = array(
            ...
            'name' => $this->input->post("wallet_name"),
        );
